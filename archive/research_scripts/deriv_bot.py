"""
Deriv API Live Trading Bot — Step Index (stpRNG)
...
PARTIAL_R         = 2.0   # Close first contract when price reaches 2R
PARTIAL_PCT       = 0.50  # Fraction of position closed at partial target
...
class BotState:
    def __init__(self):
        ...
        self.m15_candles       = []
        self.m5_candles        = []
        self.new_m5_close      = False  # flag: a new M5 candle just closed
...
# ── DERIV CLIENT ──────────────────────────────────────────────────────────────
class DerivClient:
    """Manages WebSocket with separated receiver task so send() never deadlocks."""
...
def calc_stake(atr: float, entry_price: float, risk_amount: float) -> float:
    """Stake so that 1×ATR move = risk_amount loss at current multiplier."""
...
def in_session() -> bool:
    ...
    return SESSION_START <= h < SESSION_END and h not in SKIP_HOURS
...
def handle_ohlc(msg: dict):
    ...
    if lst and lst[-1]["epoch"] == c["epoch"]:
        lst[-1] = c  # update current incomplete candle
    else:
        lst.append(c)  # new candle opened → previous just closed
        if gran == M5_GRAN:
...
async def manage_open_trade():
    ...
    # Check if partial contract was closed externally by Deriv SL (before we reach 2R)
    ...
    # Main contract closed by Deriv (SL hit or cancelled)
    ...
    # Partial close: sell partial contract when price reaches PARTIAL_R
    if not at.get("partial_done") and at.get("contract_id_partial"):
...
async def check_signal_and_trade():
    ...
    # Exclude current incomplete candle from signal calc
    ...
    # Open partial contract first (will be closed at 2R)
...
async def bot_loop():
    while True:
        try:
            ...
            while True:
                # If receiver task died, force reconnect
                ...
                # Account kill switch
                if state.balance < state.peak_balance * (1 - ACCOUNT_DD_LIMIT):
                ...
                # Only enter on new M5 candle close
                if not state.new_m5_close:
                    continue
                state.new_m5_close = False

                # Write state every cycle so dashboard stays live
                write_state()
...
def main():
    if not TOKEN:
        ...
        sys.exit(1)
    init_csv()
    log.info("Starting Deriv bot | symbol=%s | multiplier=%dx | session=%02d:00-%02d:00 GMT",
             SYMBOL, MULTIPLIER, SESSION_START, SESSION_END)
    try:
